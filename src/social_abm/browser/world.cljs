(ns social-abm.browser.world
  "Grid-based world implementation for ClojureScript"
  (:require [social-abm.browser.protocols :refer [World] :as p]))
...
(defrecord GridWorld [width height cells agents tick]
  ...
  (step-world [world]
    (-> world
        (update :tick inc)
        (assoc :agents (map #(p/step % world) agents)))))
